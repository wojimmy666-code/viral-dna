...
export const SkillShotNavigation = memo(function SkillShotNavigation({ shots, discarded, selectedId, busy, items, handlers }) {
  return <aside className="shot-navigation-panel">
    ...
    <div className="skill-shot-list">{shots.map(({ plan }, index) => <div key={plan.id} className={`skill-shot-row ${selectedId === plan.id ? "active" : ""}`} draggable={!busy} onDragStart={() => handlers.current.drag(plan.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => handlers.current.drop(plan.id)}>
      <button className="skill-shot-select" aria-current={selectedId === plan.id ? "true" : undefined} onClick={() => handlers.current.select(plan.id)} type="button"><span><strong>分镜 {String(plan.index).padStart(2, "0")}</strong><small>{shotImageStatus(plan, items)}</small></span><small>{Number(plan.start_seconds).toFixed(1)}–{Number(plan.end_seconds).toFixed(1)} 秒</small></button>
