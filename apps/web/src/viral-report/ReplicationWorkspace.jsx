...
import { CreativeIdeaEditor } from "./CreativeIdeaEditor.jsx";
...
export function ReplicationWorkspace({ analysisId, recordId, request, onPublished, onPromptPreview, onNotice, onManageCategories }) {
  ...
  const idea = ideaBatch?.ideas?.find((item) => item.id === selectedId && ideaReviewState(item, effectiveFeedback) === "ready");
  const reviewCount = ideaBatch?.ideas?.filter(item => ideaReviewState(item, effectiveFeedback) !== "ready").length || 0;
  const busy = pending || Boolean(activeJob) || Boolean(publishingId) || Boolean(editing);
  ...
  async function submit(url, body, retry = false) {
    ...
    try {
      ...
      if (operation.url.endsWith("/edit")) setEditing(null);
  ...
  return <div className="viral-report-page replication-workspace">
    ...
    <fieldset className="creative-category-lock" disabled={Boolean(editing)}><CategoryProfilePicker onChange={setSelectedCategoryId} onManage={onManageCategories} request={request} value={selectedCategoryId} /></fieldset>
    <label className="creative-feedback"><span>补充想法</span><textarea disabled={Boolean(editing)} value={effectiveFeedback} maxLength={2000} rows={2} onChange={(event) => setFeedback(event.target.value)} placeholder="例如：保留冷光与快速硬切，希望画面有想象力，不要直接推销商品。" /></label>
    ...
    {history.length > 0 && <label className="creative-history"><span>历史批次</span><select value={currentId} disabled={pending || Boolean(publishingId) || Boolean(editing)} onChange={(event) => { setCurrentId(event.target.value); setFeedback(null); }}>{history.map((item) => <option key={item.id} value={item.id}>{batchLabel(item)} · {item.category_profile?.display_name || "旧版"}</option>)}</select></label>}
    ...
    {ideaBatch && display?.phase === "ideas" && <p className="creative-review-summary" role="status">已生成 {ideaBatch.ideas.length} 条创意{reviewCount ? "，" + reviewCount + " 条待核对或修订" : "，可选择方向展开"}。{current.status === "failed" && "原批次失败记录和费用保留，下方展示已保存的内容。"}</p>}
    {current?.phase === "expanded" && (current.status === "completed" || current.error_code === "creative_prompt_language_invalid") && current.concepts?.length > 0 && <button className="text-button" type="button" onClick={() => onPromptPreview?.(current)}>{current.language_issues?.length ? "查看方案提示词并转为中文" : "查看方案提示词"}</button>}
    {ideaBatch && display?.phase !== "expanded" && <CreativeIdeas key={ideaBatch.id} ideas={ideaBatch.ideas} brief={effectiveFeedback} selectedId={selectedId} onSelect={setSelectedId} busy={busy || Boolean(retryRequest)} onRegenerate={(item) => actOnIdea(item, "regenerate")} onEdit={item => setEditing({ idea: item, batch: ideaBatch, feedback: effectiveFeedback })} />}
    {editing && <CreativeIdeaEditor key={editing.idea.id} context={editing} busy={pending || Boolean(retryRequest)} onCancel={() => setEditing(null)} onSave={body => submit("/viral-concept-sets/" + editing.batch.id + "/ideas/" + editing.idea.id + "/edit", body)} />}
