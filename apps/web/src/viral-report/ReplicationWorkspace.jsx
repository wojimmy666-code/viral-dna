...
import { CircleNotch, MagicWand, ShieldWarning, Stop } from "@phosphor-icons/react";
...
export function ReplicationWorkspace({ analysisId, recordId, request, onPublished, onPromptPreview, onNotice, onManageCategories }) {
  ...
  function actOnIdea(item, action, revisionNotes) {
    ...
    const body = { feedback: effectiveFeedback };
  ...
  return <div className="viral-report-page replication-workspace">
    ...
    <fieldset className="creative-category-lock" disabled={locked}><CategoryProfilePicker onChange={setSelectedCategoryId} onManage={onManageCategories} request={request} value={selectedCategoryId} /></fieldset>
    ...
    {ideaBatch && display?.phase !== "expanded" && <CreativeIdeas key={ideaBatch.id} ideas={ideaBatch.ideas} selectedId={selectedId} onSelect={id => { setSelectedId(id); setConceptError(null); }} busy={busy || Boolean(retryRequest)} revisionSupported={revisionSupported} onRevisionOpenChange={setRevising} onRegenerate={(item, notes) => actOnIdea(item, "regenerate", notes)} />}
    {ideaBatch && display?.phase !== "expanded" && <div className="creative-expand-action">{!idea && <span>选择一个喜欢的方向后，再展开完整分镜</span>}<Button type="button" className="primary-button" disabled={!idea || locked} onClick={() => actOnIdea(idea, "expand")}>展开这个创意</Button></div>}
