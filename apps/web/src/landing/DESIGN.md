...
- 三段样片使用缩略图、名称与 pressed 状态；选中边框外置。每 7 秒轮换并轻微推近；手动选图停止自动播放。停靠区悬停或聚焦、首屏离开视口、标签页隐藏、菜单或演示打开时暂停。减少动态效果时关闭自动轮播和动画，保留手动选图。
- 五步流程使用单选 tablist、文字步骤与当前方向图标；支持方向键、Home、End 和 roving focus。画面旁保留“创作示意”及非实际项目数据说明。
- 演示采用原生 dialog，支持关闭按钮、Escape 和遮罩关闭，锁定背景滚动并在关闭后恢复原焦点。场景按钮有 pressed 状态；内容明确为静态分镜示意，不播放虚构视频或调用模型。
...
- **Do** 保留示意影像、静态分镜和协作状态的真实性说明。
- **Do** 维护手机可达操作、键盘焦点及减少动态效果下的手动切换。

### Don't:

- **Don't** 把 AI 示意素材描述为真实客户案例、实际视频或真实账户记录。
