...
function GroupEditor({ group, ordinal, models, revision, projectId, request, resolveUrl, onSave, onReload, onDirty, registerSave, beforeGenerate, disabled }) {
  ...
  return <details className="video-group" open>
    ...
    {running && <p role="status">{running.status === "queued" ? "排队中" : "生成中"}，完成后需要核对场景与切点。<button type="button" className="text-button" disabled={pending || disabled} onClick={cancel}>取消任务</button></p>}
