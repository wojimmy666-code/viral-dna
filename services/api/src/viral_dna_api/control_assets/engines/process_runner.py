...
class DepthProcessError(RuntimeError):
    def __init__(
        self,
        message: str,
        *,
...
class DepthProcessTimeout(DepthProcessError):
    pass


class DepthProcessCancelled(DepthProcessError):
    pass
...
class AsyncProcessRunner:
    """Runs one child process without blocking the API event loop."""
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            raise DepthProcessError(f"无法启动深度处理进程：{exc}") from exc
        ...
        stdout_task = asyncio.create_task(
            _pump_stream(process.stdout, on_stdout, stdout_tail)
        )
        stderr_task = asyncio.create_task(
            _pump_stream(process.stderr, on_stderr, stderr_tail)
        )
        ...
        try:
            ...
            if cancel_task in done and cancellation.is_set():
                ...
                raise DepthProcessCancelled("深度生成已取消")
            if wait_task not in done:
                await terminate_process_tree(process)
                tail = "".join(stderr_tail or stdout_tail)
                raise DepthProcessTimeout(
                    f"深度推理超过 {timeout_seconds} 秒仍未完成",
                    output_tail=tail,
                )
