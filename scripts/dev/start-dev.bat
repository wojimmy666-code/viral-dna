...
if exist "%LOCAL_ENV_FILE%" (
  for /f "usebackq eol=# tokens=1,* delims==" %%A in ("%LOCAL_ENV_FILE%") do (
    ...
    if /I "%%A"=="VIRAL_DNA_SHOT_FACTS_CONCURRENCY" set "VIRAL_DNA_SHOT_FACTS_CONCURRENCY=%%B"
...
set "API_STATE_HELPER=%PROJECT_ROOT%\scripts\dev\api-service-state.ps1"
set "WEB_RUNNING=0"
set "API_RUNNING=0"
...
call :prepare_web
if errorlevel 1 goto :failed
node "%PROJECT_ROOT%\scripts\dev\managed-launcher.mjs"
