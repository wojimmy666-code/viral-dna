...
> 技术设计与实现记录：包含历史验收及后续规划，不能将全文视为当前已交付能力。当前用户流程见 [现行功能说明](../../features/README.md)；部署见 [部署索引](../../deployment/README.md)。

更新时间：2026-09-04

状态：Batch 5.1–5.12 已实施；持续进行真实项目验收
...
## 25. Batch 5.7–5.12：案例级大纲与结构化分镜

本批以 18.15 秒、17 镜头的工业工艺品牌片作为质量案例，但只提炼镜头语法、节奏、摄影、声音、剪辑和质量规则，不复制案例品牌、产品、文案或专有事实。新建的“电影感产品故事”项目使用 `viraldna.video-skill/v2`；已有项目继续读取创建时保存的 Skill 快照。

### Batch 5.7：Skill v2 声明契约

- Narrative 新增镜头密度、成片时长范围和 17 类镜头原型；
- 新增 Editing、Typography、Grounding、Quality 和 CanonicalCase；
- 明确 70% 细节／30% 环境、15–18 镜头、24fps、硬切和 124 BPM；
- 案例品牌词和产品特征只作为防复制规则，不作为模板变量注入。

### Batch 5.8：真实文案模型路由

- 大纲与分镜编排使用 `PROMPT_GENERATION` 路由；
- 尊重 Run Contract 的显式文案模型选择和账户默认／任务覆盖；
- 显式选择模型时关闭静默 fallback；没有可用远程路由时使用声明式本地导演编译器；
- Shot Manifest 保存实际 provider、model 和 request id。

### Batch 5.9：结构化导演中间层

- 大纲保存观众收获、内容单元、推荐镜头数、节奏、转场和证据引用；
- 每个镜头保存场景、主体、起止状态、动作阶段、镜头、光线、色彩、同步拟音、剪辑落点和连续性锁；
- 图片提示词只描述可生成的静态首帧；
- 视频提示词独立包含首帧绑定、统一视觉锁、本镜动作、同步音效、剪辑落点和严格约束；
- 成片时长按整数帧分配，模型生成时长单独映射到所选视频模型能力。

### Batch 5.10：逐镜质量门禁

- 对章节完整度、静态首帧、镜头参数、动作幅度、同步声音、一镜到底、提示词长度和案例泄漏进行评分；
- 默认最低分 85；失败镜头最多自动重写两次；
- 单镜 AI 优化支持锁定主体、机位、光线和连续性；
- Logo、包装文字和认证标识继续保留给确定性后期叠加。

### Batch 5.11：可观察、可恢复的生成体验

- 大纲与分镜改为后台任务；
- 页面显示阶段、百分比、文案模型和最近心跳；
- 模型长响应期间每 5 秒刷新一次心跳；
- 支持停止、失败重试和服务重启后的中断识别；
- 完成后提供连续性基准、剪辑配方、导演大纲和逐镜结构化审阅；
- 编辑采用 900ms 防抖自动保存，不再要求手动保存。

### Batch 5.12：回归与验收基线

- 黄金测试验证 18 秒项目输出 15–18 镜头且风格参数稳定；
- 验证替换成其他品牌／品类时不泄漏标准案例品牌和产品；
- 验证图片／视频提示词章节、长度和最低质量分；
- 前端测试覆盖进度、心跳、停止、模型审计、结构化字段、单镜优化和自动保存；
- 全量 API 与 Web 自动测试、设计规范检查和生产构建必须通过。
