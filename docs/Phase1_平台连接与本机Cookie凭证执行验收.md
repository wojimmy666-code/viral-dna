...
| 2 | GUI 配置入口 | 左侧“系统 → 平台连接”提供抖音、小红书独立状态、配置、验证、策略和断开操作。 |
| 3 | 独立文件导入 | 支持 Netscape `cookies.txt`，按所选平台过滤域名，拒绝空文件、错误编码、过期或无匹配 Cookie。 |
| 4 | 旧配置迁移 | 首次启动读取旧 `VIRAL_DNA_YTDLP_COOKIE_FILE`，分别提取抖音和小红书 Cookie，不覆盖已有连接。 |
...
“平台连接”位于系统导航，不占用第一期的四个主导航。页面固定显示两张平台状态卡：
...
抖音连接不会发送给小红书域名，小红书连接也不会发送给抖音域名。链接跳转到不受支持站点或其他平台时立即终止。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/api/v1/settings/platform-connections` | 查询当前账户、设备的两个平台状态 |
...
1. 打开“系统 → 平台连接”。
2. 确认抖音、小红书分别显示状态，且顶部明确“仅此设备”。
3. 配置其中一个平台，确认另一个平台状态不变。
4. 刷新页面，确认连接状态保留。

### B. 浏览器自动读取

1. 先在 Chrome/Edge/Firefox/Brave 登录抖音或小红书。
...
1. 在新建分析页粘贴抖音链接，确认输入框下显示抖音连接状态。
2. 粘贴小红书链接，确认状态切换到小红书。
...
3. 如曾配置旧 `VIRAL_DNA_YTDLP_COOKIE_FILE`，首次启动后确认两个平台按实际域名分别迁移；已有 GUI 配置不被覆盖。
