...
- Seedance 供应商托管虚拟人物目录、分镜绑定和请求映射已完成；人物身份统一来自托管演员，动作、空间与遮挡来自全场景深度视频，原始真人参考不会提交。目录配置见 [Seedance 托管演员与全场景深度绑定](../../architecture/generation/Phase2_Seedance供应商托管虚拟资产目录与分镜绑定_执行验收.md)，跨模型策略见 [全场景深度控制与资产重建](../../architecture/generation/Phase2_全场景深度控制与资产重建_执行验收.md)。
