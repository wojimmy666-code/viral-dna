...
> 部署流程 · 适用范围：模型调用所需的临时媒体中转，不是账户资产的长期存储。另见 [资产同步与容量配置](资产同步与容量配置.md)。

## 目标

当视频模型只接受公网 HTTPS 媒体地址时，ViralDNA 将工作区内的深度视频或参考视频临时上传到阿里云 OSS 私有 Bucket，并只向模型签发短期只读 URL。

本方案不公开 Bucket，不把 AccessKey 返回浏览器。上传对象属于当前账户和工作区，只负责模型调用的临时中转；账户级长期存储、服务器同步及容量配置是独立机制，见 [资产同步与容量配置](资产同步与容量配置.md)。
...
使用 ECS 时可按现有适配能力配置实例 RAM 角色，避免落盘长期 AccessKey；非 ECS 环境需单独核对凭据方案。Windows 本地使用独立 RAM 用户 AccessKey 时，由现有 DPAPI 秘密存储保存。换机器或运行用户后可能需要重新配置，见 [备份恢复](升级备份与恢复.md)。
...
使用独立 `admin` 登录后台，进入「媒体与对象存储」（`/admin/media`）：
...
- 本部署方案中，账户长期原件仍保存在服务器持久磁盘；OSS 只承接这里描述的媒体暂存。若未来把长期存储迁移到对象存储，需要单独实施，不能复用有自动删除生命周期的暂存前缀。
