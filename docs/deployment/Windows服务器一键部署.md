...
- 一个固定的专用本地／域 Windows 账户，已授予“作为服务登录”权限。不使用 LocalSystem，不在升级时切换账户；DPAPI 凭据与 Windows 身份和机器绑定。
- 操作菜单的管理员账户具备 GitHub SSH 读取权限；预先核实 GitHub 主机指纹并配置 `known_hosts`。部署使用非交互 SSH，不接受未知主机密钥，不使用 HTTPS 回退。
- 为 API 运行账户授予业务数据目录、API 配置目录的修改权限；API 保存设置会在配置目录内创建临时文件并原子替换原配置。代码、工具和部署控制器只需读／执行，运行账户不应能修改它们。
...
3. 选择 4 关闭服务。管理员配置域名解析及 80/443 网络入口；不要开放 API、probe、admin 端口。
4. 将 `SiteUrl` 改为实际的 `https://域名`，设 `AllowPublicAccess=true`，同步修改 API CORS 来源。
5. 选择 3 启动已有版本。脚本先确认 API 是密码模式且账户已初始化，再启动公网 Web；随后校验实际域名的 TLS 与发布版本响应头。
