...
> 历史归档 · 2026-09-09：保留原批次设计和验收结果，不作为当前操作或部署依据。后续变更见 [现行功能说明](../../features/README.md)；剩余规划见 [规划索引](../../roadmap/README.md)。

更新时间：2026-08-11

文档状态：Batch 4.1、Batch 4.2.1～4.2.4、Batch 4.3、Batch 4.4.1～4.4.5、Batch 4.5.1～4.5.3、Batch 4.6.1～4.6.6、独立视频剪辑模块重构以及分析记录生命周期 Batch 4.7 已完成；Batch 4.8 质量、安全与成本已正式启动，Batch 4.8.1 质量基线与黄金样本回归基础、Batch 4.8.2 跨分镜连续性质检已完成，下一步为 Batch 4.8.3 安全与合规检查；多图参考转场与跨镜头连续性已登记为候选 Batch 4.9 后续方向
...
- 所有必需视频片段 approved 后即可进入视频剪辑；裁剪、封面、音轨和基础质检在视频剪辑模块内完成，不再作为分段视频阶段的推进门禁。
- 时间线校验通过后才可渲染导出。
...
- 已接入百炼 Wan 2.7、MiniMax H3、MiniMax Hailuo 2.3／Fast 的物理隔离适配层；当前生产流程开放满足有序多图要求的百炼 Wan 2.7 R2V、MiniMax H3 与 Seedance 2.0／Fast／Mini。
- MiniMax H3 使用独立 `/v2` 多模态协议，Hailuo 使用 `/v1` 协议，两者的请求实现物理隔离。
- Seedance 2.0／Fast／Mini 使用官方 Model ID 与独立 Adapter；Seedance 2.5 在当前工作流能力未完成验收前继续显示“待开放”并禁止提交。
...
#### Batch 4.5.2 扩展：分镜内多画面与有序多图参考（已完成）

- 一个分镜仍对应一次视频生成，但内部增加 `visual_beats`：每个画面拥有独立提示词、源帧、图片候选、采用状态、时间比例和到下一画面的转场语义。
- 分镜图片工作台增加图1→图N 的紧凑画面轨道，支持新增、删除、排序、独立选帧和独立生图；历史候选继续按画面保留。
- 分段视频工作台显示有序故事板；所有必需画面确认后，Gateway 才把完整 `OrderedReferenceFrame[]` 交给 Provider。
- 视频能力目录新增 `multi_image_reference`、`ordered_reference_images` 和参考图数量上下限；能力不足的模型在目录、GUI 和 Gateway 三层硬阻止。
- 百炼改用 `wan2.7-r2v-2026-06-12`，按官方 `media` 数组顺序提交 `reference_image`，提示词使用图1、图2……引用；当前最多 5 张图、2～15 秒整数、720P／1080P。
- 百炼华北 2 原价继续按 720P ¥0.60／秒、1080P ¥1.00／秒估算，并保存价格来源与版本；实际结果仍以 Provider 用量回传为准。
- Seedance 2.0／Fast／Mini 与 MiniMax H3 已按官方全能参考协议开放有序多图；Hailuo 2.3／Fast 和 Seedance 2.5 继续禁用，不做自动单图降级。
- 历史多场景提示词自动迁移为有序画面，复制分镜和版本分支保留画面结构但清除生成审批，避免复用旧任务输入。

实现和人工验收见 [分镜内多画面与有序多图视频执行验收](../../architecture/generation/Phase2_分镜内多画面与有序多图视频_执行验收.md)。
...
- 已增加候选入点／出点、封面帧提取和基础技术质检；候选原文件保持不可变，旧剪辑准备参数继续可读取。
- 2026-08-11 起，逐分镜剪辑准备面板已从分段视频页面移除；视频审批完成后可直接进入视频剪辑，裁剪、封面、音轨和质检统一在独立编辑模块中完成。
...
- 前端编辑器迁入 `apps/web/src/video-editor/`，与分段视频候选审核物理隔离。
- 时间线首次初始化可直接消费已采用的视频候选；存在匹配的旧剪辑准备记录时优先迁移其参数，旧记录不再参与门禁。
- 编辑工作台采用“监看区 + 属性检查器 + 全宽时间线”，首期提供 V1 视频、A1 原音、A2 附加音频和 T1 字幕四条受控轨道。
- 裁剪、封面提取、基础质检、音量、附加音频上传／循环、字幕和转场统一版本化保存；预览和最终导出继续绑定已保存的时间线版本。

详细实现与人工验收见 [独立视频剪辑模块重构执行验收](../../qa/editing/Phase2_独立视频剪辑模块重构_执行验收.md)。

4.6.1～4.6.5 的实现边界和验收流程见 [Batch 4.6.1～4.6.5：剪辑时间线与低清预览执行验收](../../qa/editing/Phase2_Batch4.6.1-4.6.5_剪辑时间线与低清预览执行验收.md)。

4.6.6 的实现边界和验收流程见 [Batch 4.6.6：最终高清渲染与导出执行验收](../../qa/editing/Phase2_Batch4.6.6_最终高清渲染与导出执行验收.md)。
...
详细实现和人工验收见 [Batch 4.7：分析记录生命周期与批量管理执行验收](../../qa/analysis/Phase2_Batch4.7_分析记录生命周期与批量管理执行验收.md)。

### Batch 4.8：质量、安全与成本

- 当前状态：进行中；Batch 4.8.1 已建立零费用黄金样本契约、确定性检查器和 CLI，Batch 4.8.2 已完成跨分镜连续性质检，下一步为 Batch 4.8.3 安全与合规检查。
- 跨镜头人物、服装、产品和场景连续性检查。
- 黄金样本回归和全链路恢复测试。
- 人物肖像和素材权利审计。
- 工作区配额、归档、删除和恢复。
...
详细执行顺序、契约和验收标准见 [Batch 4.8：质量、安全与成本执行计划](../../roadmap/Phase2_Batch4.8_质量安全与成本执行计划.md)。

### 后续方向：多图参考转场与跨镜头连续性（候选 Batch 4.9，未实施）

- 将转场从普通时间线参数升级为连接相邻分镜的第一类生产节点。
- 以分镜 A 尾部多帧、分镜 B 目标首帧、原视频转场片段和人物／产品／服装／场景资产组成角色化参考包。
- 优先采用“中间关键帧人工确认＋多参考桥接视频”的两阶段生成；首尾帧接力和提示词约束作为模型能力不足时的降级方案。
- 保存结构化连续性协议，约束人物身份、动作阶段、画面位置、运动方向、镜头轴线、灯光和场景变化。
- 为不同模型声明首尾帧、多图、参考视频、视频延长等能力，由独立 Provider Adapter 映射厂商参数，禁止静默丢弃参考输入。
- 桥接视频保留多个候选、历史、费用和质量报告；相邻分镜变化只使对应转场过期，不删除旧产物。
- 已确认桥接片段进入受控时间线，继续复用现有音轨映射、账户通知、预览和最终渲染能力。

完整方案见 [Phase 2 后续方向：多图参考转场与跨镜头连续性](../../roadmap/Phase2_后续方向_多图参考转场与跨镜头连续性.md)。候选 Batch 4.9 在正式启动前仍需确认首个国内模型、API 能力、价格和人工验收样本。
...
- [x] 分镜内多画面、独立图片候选与有序多图视频扩展
- [ ] Batch 4.8 质量、安全与成本（进行中：4.8.1、4.8.2 已完成，下一步 4.8.3）
- [ ] 候选 Batch 4.9 多图参考转场与跨镜头连续性（后续方向，尚未排期）
...
详细实现边界见 `docs/qa/generation/Phase2_Batch4.2_图片生成双模式执行验收.md` 和 `docs/archive/phase-2/Phase2_Batch4.4_账户工作区与混合资产库执行计划.md`；人工步骤见 `docs/archive/phase-2/Phase2_Batch4.4.4_4.4.5与4.2.4_人工验收.md`。
...
详细实现与人工验收步骤见 `docs/archive/phase-2/Phase2_Batch4.5.1_视频生成基础架构执行验收.md`。
...
- Seedance 2.0／Fast／Mini 已按官方 API 与 Model ID 开放；Seedance 2.5 继续保持禁用，等待当前多图工作流完成能力验收。
- 设置页支持默认视频模型、分辨率和三家 Key；分镜页支持单次模型切换、费用提示、结果模型标签和余额错误。
- 视频 `local_tool` 仍只保留内部接口，不进入公共 API 或 GUI。

详细架构、模型状态、真实测试记录和人工验收步骤见 `docs/qa/generation/Phase2_Batch4.5.2_国内视频模型远程API执行验收.md`。
