...
- Seedance 供应商托管虚拟人物目录、分镜绑定和 `asset://` 请求映射已完成；Seedance 的人物身份统一来自托管演员，原始真人参考不会提交。动作／构图可选用身份去除校验通过的图片或视频白模。目录配置见 [Seedance 供应商托管虚拟资产目录与分镜绑定](./Phase2_Seedance供应商托管虚拟资产目录与分镜绑定_执行验收.md)，跨模型策略见 [视频人物参考策略与图片／视频白模代理](./Phase2_视频人物参考策略与图片视频白模代理_执行验收.md)。
